<?php
if ( ! class_exists( 'Wise_Blog_Social_Widget' ) ) {
	/**
	 * Adds Wise Blog Social Widget.
	 */
	class Wise_Blog_Social_Widget extends WP_Widget {

		/**
		 * Register widget with WordPress.
		 */
		public function __construct() {
			$wise_blog_social_widget = array(
				'classname'   => 'widget wise-blog-widget social-widget style-2',
				'description' => __( 'Retrive Social Widget', 'wise-blog' ),
			);
			parent::__construct(
				'wise_blog_social_widget',
				__( 'Artify Widget: Social Widget', 'wise-blog' ),
				$wise_blog_social_widget
			);
		}
		...
		public function form( $instance ) {
			...
			for ( $i = 1; $i <= 4; $i++ ) {
				...
				?>
				<p>
					<label for="<?php echo esc_attr( $this->get_field_id( 'link-' . $i ) ); ?>"><?php echo sprintf( esc_html__( 'Link %d :', 'wise-blog' ), $i ); ?></label>
